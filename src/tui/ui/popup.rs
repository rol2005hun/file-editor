...
pub fn render(f: &mut Frame, app: &App) -> Option<Rect> {
    match app.mode {
        AppMode::Settings(selection) => {
            ...
            let s1 = if app.config.show_line_numbers {
                "[X] Show Line Numbers"
            } else {
                "[ ] Show Line Numbers"
            };
            let s2 = if app.config.show_status_bar {
                "[X] Show Status Bar"
            } else {
                "[ ] Show Status Bar"
            };
            let s3 = if app.config.show_help_bar {
                "[X] Show Help Bar"
            } else {
                "[ ] Show Help Bar"
            };
            let s4 = if app.config.highlight_active_line {
            ...
            for (i, opt) in options.iter().enumerate() {
                if i == selection {
                    items.push(
                        ListItem::new(Line::from(format!("> {}", opt)))
